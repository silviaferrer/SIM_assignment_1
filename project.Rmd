...
We asses a normality test to the target variable. We reject null hypothesis (H0: variable is normal), therfore we shall not consider that the target variable is normally distributed.
```{r}
...
## Variable analysis
# Analysis functions
To proceede with the predictor analysis, we shall create the following functions:
```{r}
...
# analysis with the target variable
ggplot(train2, aes(x=SalePrice, fill=Foundation)) +
 geom_density(alpha=.5)
...
# analysis with the target variable
ggplot(train2, aes(x=SalePrice, fill=GarageType)) +
 geom_density(alpha=.5)
...
# analysis with the target variable
ggplot(train2, aes(x=SalePrice, fill=MSSubClass)) +
 geom_density(alpha=.5)
...
First, we check which variables have NA's and then we decide whether we impute or not. 
```{r}
skim(train2)
...
We exclude the imputation of the GarageYrBlt feature because it has no sense to impute this variable. 
```{r}
...
```
The imputation doesn't change much the density nor summary. 
...
```
The imputation doesnt change much the density nor summary
...
```{r}
max(imputed_data$univ_outl_count) #10
...
display(n00<- lm(log(SalePrice) ~ LotArea + OverallQual + OverallCond + YearBuilt +
...
plot(n00)
influencePlot(n00)
corrplot(descrCor, method = 'number') # we cannot see anything
...
display(n0<- lm(log(SalePrice) ~  LotArea + OverallQual + OverallCond + YearBuilt +
...
display(v0<- lm(log(SalePrice) ~  log(LotArea) + OverallQual + OverallCond + YearBuilt +
                  BsmtFinSF1  + BsmtFinSF2+ X1stFlrSF+ X2ndFlrSF +
                  BsmtFullBath + KitchenAbvGr + Fireplaces + GarageCars  +
                  ScreenPorch + PoolArea, data = train_new))
...
display(v1<- lm(log(SalePrice) ~  log(LotArea) + OverallQual + OverallCond + YearBuilt +
...
Therefore we have 2 potential models. We have been looking on the accuracy of the model, currently on 87% (will deppend on the seed), but we need to check if there is overfitting using the test sample. 
...
```
Seems that the model has no overfitting. No significant difference on the measures of the model between the train sample and the test.  
