...
mod.fow <- stats::step(lm(formula, data = imputed_data), trace = FALSE,
                       direction = "forward")
summary(mod.fow)

display(m0 <- lm(SalePrice ~ 1, data = imputed_data))
display(m1 <- lm(SalePrice ~LotArea , data = imputed_data))
anova(m0,m1) # rss
as.matrix(AIC(m0, m1))

display(m2 <- lm(SalePrice ~ LotArea+ OverallQual , data = imputed_data))
anova(m1,m2)
as.matrix(AIC(m1, m2))
display(m3 <- lm(SalePrice ~LotArea+ OverallQual+OverallCond , data = imputed_data))
anova(m2,m3)
as.matrix(AIC(m2, m3))
# afegir OverallCond no millora significativament -> ens quedem amb el model més senzill.

display(m4<- lm(SalePrice ~LotArea+ OverallQual+BsmtFinSF1 , data = imputed_data))
anova(m2,m4)
as.matrix(AIC(m2, m4)) # rss si millora, aic no. 

display(m5<- lm(SalePrice ~LotArea+ OverallQual+BsmtFinSF1+ BsmtFinSF2  , data = imputed_data))
anova(m4,m5) # no milllora model
as.matrix(AIC(m4, m5)) # no
