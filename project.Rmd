...
ggplot(train2, aes(x=SalePrice, fill=ExterQual)) +
 geom_density(alpha=.5)
ggplot(train2, aes(y=SalePrice, fill=ExterQual)) +
...
ggplot(train2, aes(x=SalePrice, fill=BsmtQual)) +
 geom_density(alpha=.5)
ggplot(train2, aes(y=SalePrice, fill=BsmtQual)) +
 geom_boxplot(alpha=0.5)
...
ggplot(train2, aes(x=SalePrice, fill=KitchenQual)) +
 geom_density(alpha=.5)
ggplot(train2, aes(y=SalePrice, fill=KitchenQual)) +
 geom_boxplot(alpha=0.5)
pairwise.wilcox.test(train2$SalePrice, train2$KitchenQual)

ks.test(train2$SalePrice[train2$KitchenQual=="TA"],train2$SalePrice[train2$KitchenQual=="Fa"]) # reject
```
...
ggplot(train2, aes(x=SalePrice, fill=GarageFinish)) +
 geom_density(alpha=.5)
...
ggplot(train2, aes(x=SalePrice, fill=FireplaceQu)) +
 geom_density(alpha=.5)
...
# Correlation Matrix
When we are analysing the instances which have a count of univariate outliers higher than 2, a total of 10 instances, we can see that BsmtFinSF2 values are generally above the mean, and the same happens with the ScreenPorch and the PoolArea.
...
imputed_data[which(imputed_data$univ_outl_count > 2),]
...
Filter variables in order to have only those with similar distribution of values. For that reason, we have filtered the values that had more unique values than 40. Then, we have excluded the variables that were centered in the '0' value.
...
```{r}
nearZeroVar(imputed_data, saveMetrics= TRUE)
nzv <- nearZeroVar(imputed_data)
nzv
prdata<-imputed_data[,-nzv]
```

We start by building the model with a manual stepwise technique, we will check the R^2 and AIC metrics to select the better model. 
We can compare with automatic forward stepwise of the function step()
```{r}
# We start with the constant and adding each chunk a variable
display(m0 <- lm(log(SalePrice) ~ 1, data = imputed_data))
display(m1 <- lm(log(SalePrice) ~LotArea , data = imputed_data))
anova(m0,m1) # rss
as.matrix(AIC(m0, m1)) 

display(m2 <- lm(log(SalePrice) ~ LotArea+ OverallQual , data = imputed_data))
anova(m1,m2)
as.matrix(AIC(m1, m2))

display(m3 <- lm(log(SalePrice) ~LotArea+ OverallQual+OverallCond , data = imputed_data))
anova(m2,m3) # no millora r^2
as.matrix(AIC(m2, m3)) 

display(m4 <- lm(log(SalePrice)~LotArea+ OverallQual+YearBuilt , data = imputed_data))
anova(m2,m4) ## seems to improve the RSS but not the R^2, 
as.matrix(AIC(m2, m4))

display(m5<- lm(log(SalePrice) ~LotArea+ OverallQual+YearBuilt +YearRemodAdd , data = imputed_data))
anova(m4,m5)
summary(m5) ## seems to improve RSS but not R^2
as.matrix(AIC(m4, m5)) 

display(m6<- lm(log(SalePrice)~LotArea+ OverallQual+ YearBuilt +YearRemodAdd+MasVnrArea , data = imputed_data))
anova(m5,m6) 
as.matrix(AIC(m5, m6)) 

display(m7<- lm(log(SalePrice) ~LotArea+ OverallQual+ YearBuilt +YearRemodAdd+MasVnrArea+BsmtFinSF1 , data = imputed_data))
anova(m6,m7) 
as.matrix(AIC(m6, m7)) 


display(m8<- lm(log(SalePrice) ~LotArea+ OverallQual+ YearBuilt +YearRemodAdd+MasVnrArea+BsmtFinSF1+BsmtUnfSF , data = imputed_data))
anova(m7,m8) # seems to improve RSS but not R^2
as.matrix(AIC(m7, m8)) 

display(m9<- lm(log(SalePrice) ~LotArea+ OverallQual+ YearBuilt +YearRemodAdd+MasVnrArea+BsmtFinSF1+BsmtUnfSF +X1stFlrSF, data = imputed_data))
anova(m8,m9) 
summary(m8)
as.matrix(AIC(m8, m9)) 

display(m10<- lm(log(SalePrice) ~LotArea+ OverallQual+ YearBuilt +YearRemodAdd+MasVnrArea+BsmtFinSF1+BsmtUnfSF +X1stFlrSF+X2ndFlrSF, data = imputed_data))
anova(m9,m10) 
summary(m10)
as.matrix(AIC(m9, m10)) 

display(m11<- lm(log(SalePrice) ~LotArea+ OverallQual+ YearBuilt +YearRemodAdd+MasVnrArea+BsmtFinSF1+BsmtUnfSF+X1stFlrSF+X2ndFlrSF+LowQualFinSF, data = imputed_data))
anova(m10,m11) 
summary(m11)
as.matrix(AIC(m10, m11))  # not much better


display(m12<- lm(SalePrice ~LotArea+ OverallQual+ YearBuilt +YearRemodAdd+MasVnrArea+BsmtFinSF1+BsmtUnfSF +X1stFlrSF+X2ndFlrSF+BsmtFullBath, data = imputed_data))
anova(m10,m12) 
summary(m12) # r^2 not better
as.matrix(AIC(m10, m12)) 


display(m13<- lm(log(SalePrice )~LotArea+ OverallQual+ YearBuilt +YearRemodAdd+MasVnrArea+BsmtFinSF1+BsmtUnfSF +X1stFlrSF+X2ndFlrSF+BedroomAbvGr, data = imputed_data))
anova(m10,m13) 
summary(m13) # not better 
as.matrix(AIC(m10, m13))

display(m14<- lm(log(SalePrice) ~LotArea+ OverallQual+ YearBuilt +YearRemodAdd+MasVnrArea+BsmtFinSF1+BsmtUnfSF +X1stFlrSF+X2ndFlrSF+KitchenAbvGr+GarageCars, data = imputed_data))
anova(m13,m14) 
summary(m14)
as.matrix(AIC(m13, m14))


# Area variable to logaritmic scale
display(m15<- lm(log(SalePrice) ~LotArea+ OverallQual+ YearBuilt +YearRemodAdd+MasVnrArea+BsmtFinSF1+BsmtUnfSF +X1stFlrSF+X2ndFlrSF+KitchenAbvGr+GarageCars, data = imputed_data))
anova(m14,m15) 
summary(m15) 
...
mod.fow <- stats::step(lm(SalePrice~. , data = prdata), trace = FALSE,
                       direction = "forward")
summary(mod.fow)


display(n00<- lm(SalePrice ~ LotArea + OverallQual + OverallCond + YearBuilt +
            MasVnrArea + BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF +
            X1stFlrSF + X2ndFlrSF  +
            BsmtFullBath + FullBath + HalfBath + BedroomAbvGr +
            KitchenAbvGr + Fireplaces + GarageCars  +
            OpenPorchSF + X3SsnPorch + ScreenPorch +
            PoolArea, data = imputed_data))

# logaritmic scale: 
display(n0<- lm(log(SalePrice) ~ log(LotArea) + OverallQual + OverallCond + YearBuilt +
            MasVnrArea + BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF +
            X1stFlrSF + X2ndFlrSF  +
            BsmtFullBath + FullBath + HalfBath + BedroomAbvGr +
            KitchenAbvGr + Fireplaces + GarageCars  +
...
display(n02<- lm(log(SalePrice) ~ log(LotArea) + OverallQual + OverallCond + YearBuilt +
...
display(n0<- lm(log(SalePrice) ~ log(LotArea) + OverallQual + OverallCond + YearBuilt +
            ...
            X1stFlrSF + X2ndFlrSF  +
            BsmtFullBath + FullBath + HalfBath + BedroomAbvGr +
            KitchenAbvGr + Fireplaces + GarageCars, data = imputed_data))

display(n1<- lm(log(SalePrice) ~ log(LotArea) + OverallQual + OverallCond + YearBuilt +
            MasVnrArea + BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF +
            X1stFlrSF + X2ndFlrSF  +
            BsmtFullBath + FullBath + HalfBath + BedroomAbvGr +
            KitchenAbvGr + Fireplaces + GarageCars  +
            OpenPorchSF + X3SsnPorch + ScreenPorch +
            PoolArea+ Neighborhood, data = imputed_data))

anova(n0,n1) # rss #####
as.matrix(AIC(n0, n1))

display(n2 <- lm(SalePrice ~ LotArea + OverallQual + OverallCond + YearBuilt +
            MasVnrArea + BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF +
            X1stFlrSF + X2ndFlrSF  +
            BsmtFullBath + FullBath + HalfBath + BedroomAbvGr +
            KitchenAbvGr + Fireplaces + GarageCars  +
            OpenPorchSF + X3SsnPorch + ScreenPorch +
            PoolArea + Neighborhood +ExterQual, data = imputed_data))
anova(n1,n2) # rss
as.matrix(AIC(n1, n2))

display(n3 <- lm(SalePrice ~ LotArea + OverallQual + OverallCond + YearBuilt +
            MasVnrArea + BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF +
            X1stFlrSF + X2ndFlrSF  +
            BsmtFullBath + FullBath + HalfBath + BedroomAbvGr +
            KitchenAbvGr + Fireplaces + GarageCars  +
            OpenPorchSF + X3SsnPorch + ScreenPorch +
            PoolArea + Neighborhood +ExterQual+MSSubClass, data = imputed_data))
anova(n2,n3) # rss
as.matrix(AIC(n2, n3))
...
ggplot(imputed_data, aes(x=SalePrice, fill=Neighborhood)) +
 geom_density(alpha=.5)

ggplot(imputed_data, aes(y=SalePrice, fill=Neighborhood)) +
 geom_boxplot(alpha=0.5)

pairwise.wilcox.test(imputed_data$SalePrice, imputed_data$Neighborhood)

```


```{r}
ggplot(train2, aes(x=SalePrice, fill=Neighborhood)) +
 geom_density(alpha=.5)
